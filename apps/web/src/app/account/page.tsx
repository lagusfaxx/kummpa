...
function IcoBox() {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><polyline points="21 8 21 21 3 21 3 8" /><rect x="1" y="3" width="22" height="5" /><line x1="10" y1="12" x2="14" y2="12" /></svg>;
}
function IcoCheck() {
...
function TabPedidos({ accessToken }: { accessToken: string }) {
  ...
  useEffect(() => {
    ...
    void listOrders(accessToken, { role: "buyer", limit: 30 })
      .then(setOrders)
      .catch(() => null)
  ...
  if (orders.length === 0) {
    return (
      <div className="overflow-hidden rounded-3xl border border-[hsl(var(--border))] bg-white p-10 text-center shadow-sm">
        <p className="text-5xl">📦</p>
        <p className="mt-4 font-bold text-slate-800">Aún no tienes pedidos</p>
        <p className="mt-1 text-sm text-slate-500">Explora tiendas cercanas y haz tu primera compra.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {orders.map(order => (
        <div key={order.id} className="overflow-hidden rounded-3xl border border-[hsl(var(--border))] bg-white shadow-sm">
          <button
...
export default function AccountPage() {
  ...
  const [tab, setTab] = useState<Tab>(() => {
    ...
    if (t === "pedidos" || t === "reservas" || t === "config") return t;
    return "perfil";
  });
      ...
      <div className="mx-auto max-w-2xl pb-16 pt-2">
        ...
        {loading ? (
          ...
          <>
            ...
            {tab === "pedidos" && (
              <TabPedidos accessToken={accessToken} />
            )}
            {tab === "config" && (
